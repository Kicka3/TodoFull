...
test("correct todolist should be added", () => {
  ...
  const endState = todolistsReducer(
    startState,
    todolistsThunks.addTodolist.fulfilled({ todolist }, "requestID", todolistId1),
  );
...
test("correct todolist should change its name", () => {
  ...
  const ActionArgs = { id: todolistId2, title: newTodolistTitle };

  const endState = todolistsReducer(
    startState,
    todolistsThunks.changeTodolistTitle.fulfilled(ActionArgs, "requestID", ActionArgs),
  );
